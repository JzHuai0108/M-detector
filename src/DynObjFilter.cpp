...
void  DynObjFilter::filter(PointCloudXYZI::Ptr feats_undistort, const M3D & rot_end, const V3D & pos_end, const lio::Time& scan_end_time)
{
    ...
    {
        Cluster.Clusterprocess(dyn_tag_cluster, *laserCloudDynObj, raw_points_world, rot_end, pos_end);
        for(int i = 0; i < size; i++)
        {   
            PointType po;
