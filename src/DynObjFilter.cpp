...
void  DynObjFilter::filter(PointCloudXYZI::Ptr feats_undistort, const M3D & rot_end, const V3D & pos_end, const ros::Time& scan_end_time)
{
    ...
    header_clus.stamp = scan_end_time;
    ...
    Buffer2DepthMap(scan_end_time.toSec());
    if(time_file != "") time_out << omp_get_wtime()-t3 << std::endl; //rec computation time
    size_t dyn_tag_count = 0, dyn_tag_orig_count = 0;
    if (cluster_coupled)
    {   
        for(int i = 0; i < size; i++)
        {
    ...
    if(time_breakdown_file != "") 
    {
        time_breakdown_out << total_test1 << " " << total_test2 << " "<< total_test3 << " " << total_proj << " " << total_occ << " " << total_map << std::endl;
    }
    frame_num_for_rec ++;
...
void  DynObjFilter::Buffer2DepthMap(double cur_time)
{
    ...
    {   
        ...
        {   
            ...
            {
                if(static_cast<int>(depth_map_list.size()) < max_depth_map_num)
                {
                    map_index ++;
            ...
            else if((point->time - depth_map_list.back()->time) >= depth_map_dur - frame_dur/2.0)
            {
                ...
                else if (static_cast<int>(depth_map_list.size()) < max_depth_map_num)
                {   
                    DepthMap::Ptr new_map_pointer(new DepthMap(point->rot, point->transl, point->time, map_index));
                    depth_map_list.push_back(new_map_pointer);
                }
            ...
            switch (point->dyn)
            {
                if(static_cast<int>(depth_map_list.back()->depth_map.size()) <= point->position) 
                case STATIC:
                    ...
                    if(static_cast<int>(depth_map_list.back()->depth_map[point->position].size()) < max_pixel_points)
                    {
    ...
    if (debug_en)
    {   
        for (int i = 0; i < static_cast<int>(depth_map_list.size()); i++)
        {
            for (int j = 0; j < static_cast<int>(depth_map_list[i]->depth_map.size()); j++)
            {
                for (int k = 0; k < static_cast<int>(depth_map_list[i]->depth_map[j].size()); k++)
                {
                    PointType po;
...
{   
    ...
    float cur_map_cons_depth_thr1 = std::max(cutoff_value, k_depth_max_thr1*(p.vec(2) - d_depth_max_thr1)) + map_cons_depth_thr1;
    float cur_map_cons_min_thr1 = std::max(cutoff_value, k_depth_min_thr1*(p.vec(2) - d_depth_min_thr1)) + enter_min_thr1;
    float cur_map_cons_max_thr1 = std::max(cutoff_value, k_depth_max_thr1*(p.vec(2) - d_depth_max_thr1)) + enter_max_thr1;
    if(dataset == 0 && p.is_distort) 
    {
    ...
    for (int ind_hor = -cur_map_cons_hor_num1; ind_hor <= cur_map_cons_hor_num1; ind_hor ++)
    {
        ...
        {   
            ...
            for (int j = 0; j < static_cast<int>(points_in_pixel.size()); j++)
            {
        ...
        if (!Case2MapConsistencyCheck(p_spherical, *depth_map_list[first_i], case2_interp_en))
        {
            ...
            for (int ind_hor = -occ_hor_num2; ind_hor <= occ_hor_num2; ind_hor ++)
            {
                ...
                {   
                    ...
                    if (pos_new < 0 || pos_new >= MAX_2D_N)  continue;
                    const std::vector<point_soph*> & points_in_pixel = depth_map_list[first_i]->depth_map[pos_new];                    
                    if (depth_map_list[first_i]->min_depth_all[pos_new] > p_spherical.vec(2))
                    {
                        continue;
                    }   
                    for (int k = 0; k < static_cast<int>(points_in_pixel.size()) && map_cons; k++)
                    {
...
bool  DynObjFilter::Case2Enter(point_soph & p, const DepthMap &map_info)
{
    ...
    if(map_info.depth_map[p.position].size() > 0)
    {
        ...
        depth_thr2_final = std::min(depth_thr2_final, v_min_thr2*delta_t);
    }
...
bool  DynObjFilter::Case2IsOccluded(const point_soph & p, const point_soph & p_occ)
{
    ...
    if(delta_t > 0)
    {
        float depth_thr2_final = std::min(std::max(cutoff_value, k_depth_max_thr2*(p.vec(2) - d_depth_max_thr2)) + occ_depth_thr2, v_min_thr2*delta_t);
...
bool  DynObjFilter::Case3Enter(point_soph & p, const DepthMap &map_info)
{
    ...
    float min_depth = 0;
    float depth_thr3_final = std::max(cutoff_value, k_depth_max_thr3*(p.vec(2) - d_depth_max_thr3)) + occ_depth_thr3;
    if(map_info.depth_map[p.position].size() > 0)
    {
...
bool  DynObjFilter::Case3IsOccluding(const point_soph & p, const point_soph & p_occ)
{
    ...
    if(delta_t > 0)
    {
        float depth_thr3_final = std::min(std::max(cutoff_value, k_depth_max_thr3*(p.vec(2) - d_depth_max_thr3)) + map_cons_depth_thr3, v_min_thr3*delta_t);
...
bool  DynObjFilter::Case3DepthConsistencyCheck(const point_soph & p, const DepthMap &map_info)
{
    ...
    if(all_num > 0)
    {
        if(num > 1)
        {      
            float cur_depth_thr = std::max(depth_cons_depth_thr3, k_depth3*p.vec(2));
            if(all_minus/(num-1) > cur_depth_thr)
            {
                return false;
            }      
...
void DynObjFilter::publish_dyn(const ros::Publisher & pub_point_out, const ros::Publisher & pub_frame_out, const ros::Publisher & pub_steady_points, const ros::Time& scan_end_time)
{
    if(cluster_coupled) // pubLaserCloudEffect pub_pcl_dyn_extend  pubLaserCloudEffect_depth
    {    
        // std::cout<<"Found Dynamic Objects, numbers: " << laserCloudDynObj_clus->points.size() << " Total time: " << time_total << " Average total time: "<< time_total_avr << std::endl;
    }
    else
    {
        std::cout<<"Found Dynamic Objects, numbers: " << laserCloudDynObj->points.size() << " Total time: " << time_total << " Average total time: "<< time_total_avr << std::endl;
    }
    // std::cout<<"case1 num: "<<case1_num<<" case2 num: "<<case2_num<<" case3 num: "<<case3_num<<std::endl;
    case1_num = 0;
    case2_num = 0;
    case3_num = 0;
    sensor_msgs::PointCloud2 laserCloudFullRes3;
    pcl::toROSMsg(*laserCloudDynObj_world, laserCloudFullRes3);
    laserCloudFullRes3.header.stamp = scan_end_time;
    ...
    if(cluster_coupled || cluster_future)
    {
        ...
        laserCloudFullRes4.header.stamp = scan_end_time;
    ...
    if(cluster_coupled)
    {
        if(laserCloudSteadObj_accu_times < laserCloudSteadObj_accu_limit)
        {
            ...
            for(int i = 0; i < static_cast<int>(laserCloudSteadObj_accu.size()); i++)
            {
    ...
    {
        std::cout<<"Found Steady Objects, numbers: " << laserCloudSteadObj->points.size() << std::endl;
        if(laserCloudSteadObj_accu_times < laserCloudSteadObj_accu_limit)
        {
            laserCloudSteadObj_accu_times ++;
    ...
    laserCloudFullRes2.header.stamp = scan_end_time;
    laserCloudFullRes2.header.frame_id = frame_id;
    pub_steady_points.publish(laserCloudFullRes2);
}

void DynObjFilter::set_path(std::string file_path, std::string file_path_origin)
{
    is_set_path = true;
    out_file = file_path;
